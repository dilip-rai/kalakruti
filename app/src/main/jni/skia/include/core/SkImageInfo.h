...
public:
    SkImageInfo()
            : fWidth(0), fHeight(0), fColorType(kUnknown_SkColorType), fAlphaType(kUnknown_SkAlphaType),
...
private:
    ...
    SkImageInfo(int width, int height, SkColorType ct, SkAlphaType at, SkColorProfileType pt)
            : fWidth(width), fHeight(height), fColorType(ct), fAlphaType(at), fProfileType(pt) { }
};
